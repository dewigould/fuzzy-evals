"""
Math RLVR Hyperparameter Sweep Orchestrator.

1. Runs calibrate_max_tokens.py to determine optimal max_tokens
...
def print_comparison(all_results):
    ...
    print(f"\n{'='*90}")
    print(f"SWEEP COMPARISON TABLE")
    print(f"{'='*90}")
...
def main():
    ...
    print(f"[{datetime.now():%H:%M:%S}] Step 0: Calibrating max_tokens...")
    ...
    if CALIBRATION_FILE.exists():
        ...
        result = subprocess.run(
            [PYTHON, str(SCRIPTS_DIR / 'calibrate_max_tokens.py')],
    ...
    print(f"\n[{datetime.now():%H:%M:%S}] Step 1: Launching {len(CONFIGS)} configs in parallel...")
    print(f"  Memory: {check_memory()}")
    ...
    for config_name in CONFIGS:
        ...
        cmd = [
            PYTHON, str(SCRIPTS_DIR / 'run_math_rlvr_single.py'),
    ...
    if all_results:
        ...
        print(f"  python3 {SCRIPTS_DIR}/eval_sweep_winner.py "
              f"--winner {winner['config']} --max_tokens {max_tokens}")
