"""Experiment 16/02 — Code distillation training.

SFT on filtered KodCode-V1-SFT-R1 into a base model.

Usage:
  # Default: 20K samples, lr=2e-4, checkpoint every 5K samples
  python train_code.py --name code_default

  # Custom settings
  python train_code.py --name code_long --samples 50000 --lr 1e-4 --save-every 200

  # Quick smoke test
  python train_code.py --name smoke --samples 500 --save-every 1 --batch-size 10
"""
...
from config import CODE_SYSTEM_PROMPT  # noqa: E402
from experiment_16_02.config import (  # noqa: E402
    ...
    SAVE_EVERY, EVAL_EVERY, TRAINING_DIR, FILTERED_CODE_DATA,
)
...
class KodCodeSFTBuilder(ChatDatasetBuilder):
    """SFT dataset builder for filtered KodCode-V1-SFT-R1."""
    ...
    def __call__(self) -> tuple[SupervisedDataset, SupervisedDataset | None]:
        ...
        if not data_path.exists():
            raise FileNotFoundError(
                ...
                "Run distillation/filter_repetitive_kodcode.py first."
            )
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 16/02: Code distillation training")
    ...
    print(f"Experiment 16/02 — Code Distillation Training")
