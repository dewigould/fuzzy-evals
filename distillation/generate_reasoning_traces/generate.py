"""Generate reasoning traces via OpenRouter.

Calls the configured model (default: Sonnet 4.5) with math problems
and saves the full responses. Resumable: skips already-processed UUIDs.

Usage:
    python generate.py [--concurrency N] [--max-tokens N] [--model MODEL] [--effortful] [--thinking]
"""
...
from config import (
    ...
    CONCURRENCY, MAX_TOKENS, SYSTEM_PROMPT, EFFORTFUL_MATH_PROMPT,
    THINKING_MODEL_MATH_PROMPT, THINKING_REASONING_TOKENS, THINKING_CONTENT_TOKENS,
    DATA_DIR, QUESTIONS_PATH, GENERATIONS_PATH,
    EFFORTFUL_DATA_DIR, EFFORTFUL_QUESTIONS_PATH, EFFORTFUL_GENERATIONS_PATH,
    THINKING_DATA_DIR, THINKING_QUESTIONS_PATH, THINKING_GENERATIONS_PATH,
)
...
def load_done_uuids(generations_path: Path) -> set[str]:
    """Load UUIDs already processed (for resume support)."""
    done = set()
...
async def generate_one(session, semaphore, question: dict, model: str,
    ...
    if thinking_mode:
        ...
        if str(reasoning).startswith("ERROR:"):
            response = reasoning  # propagate error
        else:
            # Convert reasoning + content into <think>...</think> format
            response = f"<think>\n{reasoning}\n</think>\n\n{content}"
    else:
...
async def main_async(args):
    ...
    if args.output_dir:
        ...
        system_prompt = THINKING_MODEL_MATH_PROMPT
        ...
        system_prompt = EFFORTFUL_MATH_PROMPT
        model = args.model
        print("Using EFFORTFUL math prompt")
    else:
        data_dir = DATA_DIR
    ...
    if not questions_path.exists():
        print(f"ERROR: {questions_path} not found. Run extract_questions.py first.")
        sys.exit(1)
    ...
    # Resume support
    done_uuids = load_done_uuids(generations_path)
    remaining = [q for q in questions if q["uuid"] not in done_uuids]
    if done_uuids:
...
def main():
    parser = argparse.ArgumentParser(description="Generate reasoning traces via OpenRouter")
    ...
    parser.add_argument("--effortful", action="store_true",
                        help="Use effortful math prompt and write to data_effortful/")
    parser.add_argument("--thinking", action="store_true",
                        help="Use thinking model with reasoning API and write to data_thinking/")
