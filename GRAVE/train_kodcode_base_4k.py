"""Code distillation: SFT on KodCode-V1-SFT-R1 into Qwen3-30B-A3B-Base (4K context).

Same as train_kodcode_base.py but with max_length=4096 for training.

Usage:
  python train_kodcode_base_4k.py <config_name> [--filtered]
"""
...
from config import TRAINING_BASE, LORA_RANK, CODE_SYSTEM_PROMPT
...
class KodCodeSFTBuilder(ChatDatasetBuilder):
    ...
    def _build_from_filtered(self) -> tuple[SupervisedDataset, SupervisedDataset | None]:
        ...
        ds = datasets.load_dataset("json", data_files=str(FILTERED_DATA_PATH), split="train", streaming=True)
        ...
        test_rows, remaining_rows, row_count = [], [], 0
        for row in ds:
        ...
        def train_row_generator():
            for row in remaining_rows:
                yield row
            ds2 = datasets.load_dataset("json", data_files=str(FILTERED_DATA_PATH), split="train", streaming=True)
        ...
        train_dataset = StreamingSupervisedDatasetFromHFDataset(
            hf_dataset=train_iter, batch_size=self.common_config.batch_size,
            length=self.max_prompts, map_fn=map_fn, buffer_size=self.buffer_size,
        )
        return train_dataset, FixedListDataset(test_datums, batch_size=self.test_size)
...
def main():
    parser = argparse.ArgumentParser(description="Base model KodCode distillation training (4K context)")
    ...
    task_name = "base_kodcode_4k_filtered" if args.filtered else "base_kodcode_4k"
    log_path = f"{TRAINING_BASE}/{task_name}/{config_name}"

    print(f"Base Model KodCode Distillation Training (4K context): {config_name}")
    ...
    print(f"  Batch size: {cfg['batch_size']}, Train max_length: {TRAIN_MAX_LENGTH}")
    print(f"  Log path: {log_path}")

    renderer_name = model_info.get_recommended_renderer_name(BASE_MODEL_NAME)
    common_config = ChatDatasetBuilderCommonConfig(
        model_name_for_tokenizer=BASE_MODEL_NAME, renderer_name=renderer_name,
        max_length=TRAIN_MAX_LENGTH, batch_size=cfg["batch_size"],
    )
    dataset_builder = KodCodeSFTBuilder(
        common_config=common_config, max_prompts=max_prompts,
        test_size=TEST_SIZE, use_filtered=args.filtered,
    )
    config = train.Config(
        log_path=log_path, model_name=BASE_MODEL_NAME,
        dataset_builder=dataset_builder, learning_rate=cfg["lr"],
        lr_schedule=cfg["schedule"], num_epochs=1,
        lora_rank=LORA_RANK, save_every=40, eval_every=40,
    )
