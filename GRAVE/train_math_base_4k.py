"""Math distillation: SFT on OpenR1-Math-220k into Qwen3-30B-A3B-Base (4K context).

Same as train_math_base.py but with max_length=4096 for training.
Inference still uses 28672 via eval_watcher.

Usage:
  python train_math_base_4k.py <config_name> [--filtered]

  --filtered: Use filtered_data/clean.jsonl (repetition-filtered) instead
              of raw HF dataset. Saves to training_runs/base_math_4k_filtered/.
...
from config import TRAINING_BASE, LORA_RANK, MATH_SYSTEM_PROMPT
...
def main():
    parser = argparse.ArgumentParser(description="Base model math distillation training (4K context)")
    ...
    task_name = "base_math_4k_filtered" if args.filtered else "base_math_4k"
    log_path = f"{TRAINING_BASE}/{task_name}/{config_name}"

    print(f"Base Model Math Distillation Training (4K context): {config_name}")
