"""Eval watcher for filtered math training runs.

Monitors base_math_filtered and base_math_4k_filtered for new checkpoints,
evaluates each on math_500 and aime only.

Usage:
  PYTHONUNBUFFERED=1 python eval_filtered.py
"""
...
TRAINING_RUNS = [
    {"task": "base_math_filtered", "config": "A_r32_high_lr"},
    {"task": "base_math_4k_filtered", "config": "A_r32_high_lr"},
]
...
def collect_pending_evals() -> list[dict]:
    ...
    # Sort by step so we interleave 4K and 8K jobs at each step
...
def main():
    print(f"{'='*60}")
    print(f"Filtered Math Eval Watcher")
